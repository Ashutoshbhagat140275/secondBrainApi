...
async def store_document(
    user_id: str, text: str, session_id: str, timestamp: str, emotion_label: str = None
) -> str:
    """
...
async def search_documents(
    user_id: str, query: str, top_k: int = 5, query_embedding: np.ndarray = None
) -> List[dict]:
    """
    ...
    Args:
        ...
        query_embedding: Optional pre-computed embedding (avoids re-computing)
    ...
    try:
        ...
        try:
            ...
            if collection_name not in existing_names:
                logger.info(
                    f"Collection {collection_name} does not exist - no documents uploaded yet"
                )
        ...
        if (
            ...
            logger.info(
                f"Collection for user {user_id} does not exist - no documents uploaded yet"
            )
